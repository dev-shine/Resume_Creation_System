...
const LinkEditComponent = (props) => {
  const editHandler = () => {
    designationActions.getDesignationById(props.rowData._id);
  };
  return (
...
const LinkDeleteComponent = (props) => {
  const deleteHandler = () => {
    confirm(constants.DELETE_CONFIRMATION).then(() => {
      designationActions.designationDelete(props.rowData._id);
    });
  };
  return (
    <div>
...
const linkEditData = (cell, row) => {
  return (
    <LinkEditComponent rowData={ row } moduleUrl={constants.DESIGNATION_URL} />
  );
};
...
export default class DesignationList extends React.Component {
  ...
  componentDidMount() {
    designationStore.addChangeListener(this.onDeleteChange);
  }
  onDeleteChange() {
    this.setState({ designationDeleteStatus: designationStore.getDesignationDeleteStatus() }, function() {
      if (this.state.designationDeleteStatus === constants.CONFLICT) {
        NotificationManager.error(constants.REFERENCE_EXIST_MESSAGE, '', 2000);
        designationStore.resetStatus();
      }
      else if (this.state.designationDeleteStatus === constants.OK) {
  ...
  render() {
    const options = {
      ...
      sizePerPageList: [{
        ...
        text: 'All', value: this.props.designations.length
    ...
    return (
      <div>
          <BootstrapTable className='designationGrid' data={ this.props.designations } striped hover search pagination={ true } options={ options } searchPlaceholder={ constants.FILTER }>
              ...
              <TableHeaderColumn dataField='DesignationName' dataSort={ true } width='110'>{ constants.DESIGNATION }</TableHeaderColumn>
