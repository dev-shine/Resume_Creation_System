...
const LinkEditComponent = (props) => {
  const editHandler = () => {
    operatingSystemActions.getOperatingSystemById(props.rowData._id);
  };
  return (
...
const LinkDeleteComponent = (props) => {
  const deleteHandler = () => {
    confirm(constants.DELETE_CONFIRMATION).then(() => {
      operatingSystemActions.operatingSystemDelete(props.rowData._id);
    });
  };
  return (
    <div>
...
const linkEditData = (cell, row) => {
  return (
    <LinkEditComponent rowData={ row } moduleUrl={constants.OPERATINGSYSTEM_URL} />
  );
};
...
export default class OperatingSystemList extends React.Component {
  constructor() {
    super();
    this.onDeleteChange = this.onDeleteChange.bind(this);
  }
  componentDidMount() {
    operatingSystemStore.addChangeListener(this.onDeleteChange);
  }
  onDeleteChange() {
    this.setState({ operatingSystemDeleteStatus: operatingSystemStore.getOperatingSystemDeleteStatus() }, function() {
      if (this.state.operatingSystemDeleteStatus === constants.CONFLICT) {
        NotificationManager.error(constants.REFERENCE_EXIST_MESSAGE, '', 2000);
        operatingSystemStore.resetStatus();
      }
      else if (this.state.operatingSystemDeleteStatus === constants.OK) {
  ...
  render() {
    const options = {
      ...
      sizePerPageList: [{
        ...
        text: 'All', value: this.props.operatingSystems.length
    ...
    return (
      <div>
          <BootstrapTable className='operatingSystemGrid' data={ this.props.operatingSystems } striped hover search pagination={ true } options={ options } searchPlaceholder={ constants.FILTER }>
              ...
              <TableHeaderColumn dataField='OperatingSystemName' dataSort={ true } width='110'>{ constants.OPERATINGSYSTEM }</TableHeaderColumn>
